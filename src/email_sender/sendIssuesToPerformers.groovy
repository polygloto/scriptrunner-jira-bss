...
groupList.add("bss-users")
//groupList.add("postal-group")
//groupList.add("tester-group")
def usersList = userUtil.getAllUsersInGroupNames(groupList)

usersList.each { user ->
    def jqlSearch = "status in (Open, \"In Progress\", \"To Do\", \"In Review\", \"Under Review\") AND assignee in (${user.username}) order by due asc, created asc"
    ...
    if (parseResult.isValid()) {
        ...
        if (issues.size() > 0) {
            def body = "${user.displayName}, количество ваших незакрытых задач - ${issues.size()}, просьба определить их статус. \n\n"
            def writer = new StringWriter()
            def xml = new MarkupBuilder(writer)
            String issueLink, createdDate, reporter, assignee, assigneeEmail = "", summary, status, projectKey, projectLink, due
            ...
            xml.table(id: "scriptField", style:"border-width:1px;border-style:solid;border-color:black;border-collapse:collapse;") {
                ...
                issues.each { i ->
                    ...
                    assignee = issue.getAssignee().name
                    assigneeEmail = issue.getAssignee().emailAddress
            ...
            sendEmail(assigneeEmail, emailSubject, body, user.displayName)
        } else {
...
static def sendEmail(String emailAddressee, String subject, String body, String employee) {
    ...
    if (mailServer) {
        ...
        Email supEmail = new Email("jira-rep@bi-ss.ru")
        supEmail.setMimeType("text/html")
        supEmail.setSubject(employee)
        supEmail.setBody(body)
        mailServer.send(supEmail)
